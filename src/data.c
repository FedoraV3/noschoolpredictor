...
static BOOL get_coordinates_from_ip(float *x_buf, float *y_buf) {
	...
	if (curl_easy_setopt(ip_handle, CURLOPT_URL, IP_API_SERVICE) != CURLE_OK) return FALSE;
	if (curl_easy_setopt(ip_handle, CURLOPT_WRITEFUNCTION, write_callback) != CURLE_OK) return FALSE;
	if (curl_easy_setopt(ip_handle, CURLOPT_WRITEDATA, &url_response) != CURLE_OK) return FALSE;
	if (curl_easy_perform(ip_handle) != CURLE_OK) return FALSE;
...
BOOL data_curl_init(void) {
	...
	data_easy_handle = curl_easy_init();
	
	if (curl_easy_setopt(data_easy_handle, CURLOPT_URL, buffer) != CURLE_OK) return FALSE;
	if (curl_easy_setopt(data_easy_handle, CURLOPT_WRITEFUNCTION, write_callback) != CURLE_OK) return FALSE;
...
BOOL get_weather_data(weather_data_t *data) {
	...
	if (curl_easy_setopt(data_easy_handle, CURLOPT_WRITEDATA, &url_response) != CURLE_OK) return FALSE;
	
	/* Execute */
	curl_easy_perform(data_easy_handle);
	
	/* Deserialize */
	cJSON *deserialized_response = cJSON_Parse(url_response.string);

	if (deserialized_response == NULL) return FALSE;
